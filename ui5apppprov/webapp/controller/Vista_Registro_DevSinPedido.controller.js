sap.ui.define([
	...
	return Controller.extend("nspprov.ui5apppprov.controller.Vista_Registro_DevSinPedido", {
		...
		onInit: function () {
			var oThis = this;
			oThis.getRouter().getRoute("Vista_Registro_DevSinPedido").attachMatched(this._onRouteMatched, this);
		...
		_onRouteMatched: function () {
			...
			this.getView().byId("idCodigoDetra").setEnabled(false);
								...
								success: function (response) {
									...
									if (varValidacionExiste !== "") {
										if (lenghtV === 0) {
											...
											//var varCodTipoDetraccion = oModel2.getProperty("/cac:PaymentMeans/cbc:PaymentMeansCode"); ---NO
											//var varCodTipoDetraccion = oModel2.getProperty("/cac:PaymentTerms/cbc:PaymentMeansID");
											var varCodTipoDetraccion = "";
										...
										var dialogValTipFac = new sap.m.Dialog({
											...
											icon: "sap-icon://error",
											content: new sap.m.Text({
												text: "La Factura que se Intenta Ingresar no es un tipo Nota de Credito(07)"
											}),
											beginButton: new sap.m.Button({
												text: "Aceptar",
												press: function () {
		...
		btnFacturar: function () {
			...
			if (afirmacion) {
				...
				var varValidacionDocReferencia = this.getView().byId("idDocReferencia").getValue();

				var varContNumErrores = true;
				if (varValidacionDocReferencia !== "") {
									...
									success: function (data) {
										...
										for (var a = 0; a < listItemDetalleFactura.length; a++) {
											...
											for (var b = 0; b < itemsOC.length; b++) {
												...
												llaveItem.CODIGO = "";
												llaveItem.DESCRIPCION = itemsOC[b].DE_DESCRIPCION;
					...
					var dialogValTipFac = new sap.m.Dialog({
						...
						icon: "sap-icon://error",
						content: new sap.m.Text({
							text: "Se debe Ingresar un Documento de Referencia"
						}),
						beginButton: new sap.m.Button({
							text: "Aceptar",
							press: function () {
		...
		btnAnadirDocSus: function () {
			...
			if (varSelecTipoCarga === "V") {
				varDocumentosObli = "07 - xml";
					...
					press: function () {
						...
						if (varContErrorBinario === 0) {
							if (varContNoEstado === 0) {
								if (this.getView().byId("idComboTipo").getSelectedItem().getKey() === "V") {
