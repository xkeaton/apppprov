sap.ui.define([
	...
	return Controller.extend("nspprov.ui5apppprov.controller.Vista_Registro_Contratista", {
		...
		onInit: function () {
			...
			var oThis = this;
			oThis.getRouter().getRoute("Vista_Registro_Contratista").attachMatched(this._onRouteMatched, this);
		...
		_onRouteMatched: function () {
			...
			//oModelJson2.read("/T_CTR_DETs?$format=json", {
			oModelJson2.read("/" + this.varTableT_CTR_DET + "?$format=json", {
				filters: filterOrdenCompra,
				success: function (response) {
		...
		btnBuscarItem: function (oEvent) {
			...
			llaveSeleccionada.NETOCIGV = parseFloat(oModel.getProperty(valObject + "/clistItemDetalleFacturaValortotalNetoXItem")).toFixed(2);
			llaveSeleccionada.ORDEN_COMPRA = oModel.getProperty(valObject + "/clistItemsOrdenCompra");
			llaveSeleccionada.TOTALFACTURA = this.getView().byId("idImporteTotal").getValue();
				...
				for (var l = 0; l < listItemDetalleFactura.length; l++) {
					...
					if (listItemDetalleFactura[l].clistItemDetalleFacturaPosicion !== llaveSeleccionada.POS) {
						for (var m = 0; m < listItemDetalleFactura[l].clistItemsOrdenCompra.length; m++) {
							...
							if (listaValesIngreso[k].BELNR === ordenCompraSelect.BELNR &&
								listaValesIngreso[k].GJAHR === ordenCompraSelect.GJAHR) {
								realizar = false;
							}
						}
					}
			...
			for (var i = 0; i < llaveSeleccionada.ORDEN_COMPRA.length; i++) {
				total += parseFloat(llaveSeleccionada.ORDEN_COMPRA[i].RMWWR.toString().replace(',', ''));
				for (var j = 0; j < listaValesIngreso.length; j++) {
					if (listaValesIngreso[j].BELNR === llaveSeleccionada.ORDEN_COMPRA[i].BELNR &&
						listaValesIngreso[j].GJAHR === llaveSeleccionada.ORDEN_COMPRA[i].GJAHR) {
					...
					select: function (evt) {
						...
						for (var v = 0; v < listaValesIngreso.length; v++) {
							if (listaValesIngreso[v].selectItem) {
								if (listaValesIngreso[v].BELNR === posicion) {
									var varMonedaOC = oModel.getProperty("/listItemCabeceraFactura/8/value");
									if (listaValesIngreso[v].WAERS === varMonedaOC) {
						...
						if (!realizar) {
							...
							for (var v = 0; v < listaValesIngreso.length; v++) {
								if (listaValesIngreso[v].selectItem) {
									totalVale += parseFloat(listaValesIngreso[v].RMWWR.toString().replace(',', ''), 10);
								}
							}
							var subTotal = [];
							var llaveSub = {};
				...
				template: new sap.m.ObjectStatus({
					...
					state: {
						...
						formatter: function (total) {
							...
							var valorNetoMas5 = parseFloat(myParam.getProperty("/facturaSeleccionada/0/TOTALFACTURA"));
							valorNetoMas5 = valorNetoMas5.toFixed(2);
							//var valorNetoMenos5 = parseFloat(netoxItem) - 1;
							var valorNetoMenos5 = parseFloat(myParam.getProperty("/facturaSeleccionada/0/TOTALFACTURA"));
							...
							new sap.ui.layout.VerticalLayout({
								...
								content: [
									...
									new sap.m.Input({
										maxLength: 10,
										id: "idFecVale",
										valueStateText: "El campo División no debe estar vacío.",
										placeholder: "Ingrese División (10)...",
										width: "100%"
									})
								]
							}),
							new sap.ui.layout.VerticalLayout({
								...
								content: [
									...
									new sap.m.Input({
										maxLength: 10,
										id: "idNroVale",
										valueStateText: "El campo Pre-registro no debe estar vacío.",
										placeholder: "Ingrese Pre-registro (10)...",
										width: "100%"
									})
								]
							}),
							new sap.ui.layout.VerticalLayout({
								...
								content: [
									...
									new sap.m.Input({
										maxLength: 16,
										id: "idGuia",
										valueStateText: "El campo Ejercicio no debe estar vacío.",
										placeholder: "Ingrese Ejercicio (16)...",
										width: "100%"
									})
								]
							}),
										...
										if (canContinue) {
											...
											jQuery.each(inputs, function (i, input) {
												...
												if (idCampo === "idFecVale") {
													filter = new sap.ui.model.Filter("GSBER", sap.ui.model.FilterOperator.Contains, valor);
													aFilters.push(filter);
												} else if (idCampo === "idNroVale") {
													if (comboKey === "C") {
														filter = new sap.ui.model.Filter("BELNR", sap.ui.model.FilterOperator.Contains, valor);
														aFilters.push(filter);
													}
												} else if (idCampo === "idGuia") {
													filter = new sap.ui.model.Filter("GJAHR", sap.ui.model.FilterOperator.Contains, valor);
													aFilters.push(filter);
												}
											});
			...
			var dialogMensaje = new sap.m.Dialog({
				...
				endButton: new sap.m.Button({
					press: function () {
						dialogMensaje.close();
						if (llave === "C") {
							this.getView().byId("idComboTipo").setSelectedKey("C");
						}
					}.bind(this),
					text: "Cancelar"
				}),
		...
		subTotalOrdenCompra: function (contenido) {
			var myParam = this.getView().getModel("myParam");
			var total = 0;
			var valorNeto = parseFloat(myParam.getProperty(this.posFacturaGlobal + "/clistItemDetalleFacturaValortotalNetoXItem")).toFixed(2);
			...
			for (var i = 0; i < contenido.length; i++) {
				var valorIngreso = contenido[i].RMWWR;
			...
			try {
				...
				if (afirmacion) {
					...
					var dialog = new sap.m.Dialog({
						...
						state: "Warning",
						content: new sap.m.Text({
							text: "Se requiere seleccionar alguna Factura y/o Centro."
				...
				if (file && window.FileReader) {
					...
					reader.onload = function (evn) {
						try {
							...
							var varNumFacturaValid2 = oModel2.getProperty("/n4:ID");
							...
							oModelValDocIns.read("/" + this.varTableT_FAC + "?$format=json", {
								...
								success: function (response) {
									var oModelJSON = new sap.ui.model.json.JSONModel(response.results);
									var lenghtV = oModelJSON.getData().length;
									// 20200311  || varFecEmisionValidar !== varAzoGlobalFacturar
									if (lenghtV === 0) { // quitar || lenghtV !== 0
										...
										var varVersionUBL2 = oModel2.getProperty("/n4:UBLVersionID");
										if (varVersionUBL1 !== "") {
										...
										var varNumFactura2 = oModel2.getProperty("/n4:ID");
										if (varNumFactura1 !== "") {
										...
										var varFecEmision2 = oModel2.getProperty("/n4:IssueDate");
										if (varFecEmision1 !== "") {
										...
										var rucEmisor2 = oModel2.getProperty("/cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID");
										var rucEmisor3 = oModel2.getProperty("/cac:AccountingSupplierParty/cbc:CustomerAssignedAccountID");
										var rucEmisor4 = oModel2.getProperty("/cac:AccountingSupplierParty/n5:Party/n5:PartyIdentification/n4:ID");
										var varRUCEmisor = "";
										if (rucEmisor1.toString().trim() !== "") {
											varRUCEmisor = rucEmisor1;
										} else if (rucEmisor2.toString().trim() !== "") {
										...
										if (varRUCEmisor.trim().toString() === usuarioLogin.trim().toString()) {
											...
											var rucReceptor2 = oModel2.getProperty("/cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID");
											var rucReceptor3 = oModel2.getProperty("/cac:AccountingCustomerParty/cbc:CustomerAssignedAccountID");
											var rucReceptor4 = oModel2.getProperty("/n5:AccountingCustomerParty/n5:Party/n5:PartyIdentification/n4:ID");
											var varRUCReceptor = "";
											if (rucReceptor1.toString().trim() !== "") {
												varRUCReceptor = rucReceptor1;
											} else if (rucReceptor2.toString().trim() !== "") {
											...
											if (varRUCReceptor === usuarioRuc) {
												...
												var recRazonSocial3 = oModel2.getProperty(
													"/n5:AccountingCustomerParty/n5:Party/n5:PartyLegalEntity/n4:RegistrationName");
												var varRecRazonSocial = "";
												if (recRazonSocial1.toString().trim() !== "") {
													varRecRazonSocial = recRazonSocial1;
												} else if (recRazonSocial2.toString().trim() !== "") {
												...
												if (realizar) {
													...
													if (textoVariableCodigo === "1") {
														...
														for (var j = 0; j < tamDetalleFactura; j++) {
															...
															varPosicion[j] = oModel2.getProperty(texto + "/" + j + "/n4:ID");

															valor1Codigo = oModel2.getProperty(texto + "/" + j + "/n5:Item/n5:SellersItemIdentification/n4:ID");
															valor2Codigo = oModel2.getProperty(texto + "/" + j + "/cac:Item/cac:AdditionalItemProperty/cbc:Value");
															if (valor1Codigo.toString().trim() !== "") {
																varCodigo[j] = valor1Codigo;
															} else if (valor2Codigo.toString().trim() !== "") {
															...
															varDescripcion[j] = oModel2.getProperty(texto + "/" + j + "/n5:Item/n4:Description");
															descripcionTotal = oModel2.getProperty(texto + "/" + j + "/n5:Item/cac:AdditionalItemProperty/cbc:Name");
															if (descripcionTotal.toString().trim() !== "") {
																varDescripcion[j] = varDescripcion[j] + " - " + descripcionTotal;
															}
															...
															varPrecioUnitxItem[j] = oModel2.getProperty(texto + "/" + j + "/n5:Price/n4:PriceAmount");
															varPrecioVentaxItem[j] = oModel2.getProperty(texto + "/" + j +
																"/n5:PricingReference/n5:AlternativeConditionPrice/n4:PriceAmount");
															varTotalIGVxItem[j] = oModel2.getProperty(texto + "/" + j + "/n5:TaxTotal/n4:TaxAmount");
															varValorVentaxItem[j] = oModel2.getProperty(texto + "/" + j + "/n4:LineExtensionAmount");
															varValortotalNetoXItem[j] = parseFloat(varTotalIGVxItem[j], 10) + parseFloat(varValorVentaxItem[j], 10);

															var1PorcentajeVal = oModel2.getProperty(texto + "/" + j + "/n5:TaxTotal/n5:TaxSubtotal/n5:TaxCategory/n4:Percent");
															var2PorcentajeVal = oModel2.getProperty(texto + "/" + j +
																"/cac:TaxTotal/cac:TaxSubtotal/cac:TaxCategory/cbc:Percent");
															if (var1PorcentajeVal.toString().trim() !== "") {
																varValorPorcentajeIGV[j] = var1PorcentajeVal;
													...
													if (textoVariableCodigo === "1") {
														...
														var varTotalDescuentos = oModel2.getProperty("/n5:LegalMonetaryTotal/cbc:AllowanceTotalAmount");
														var varTotalIGV = oModel2.getProperty("/n5:TaxTotal/n4:TaxAmount");
														var varImporteTotal = oModel2.getProperty("/n5:LegalMonetaryTotal/n4:PayableAmount");
													}
								...
								success: function (data) {
									...
									for (var a = 0; a < listItemDetalleFactura.length; a++) {
										...
										for (var b = 0; b < itemsOC.length; b++) {
											...
											//llaveItem.DE_POSICION = itemsOC[b].GJAHR;
											if (itemsOC[b].GJAHR !== "" && itemsOC[b].GJAHR !== null && itemsOC[b].GJAHR !== undefined) {
												llaveItem.DE_POSICION = itemsOC[b].GJAHR;
											} else {
												llaveItem.DE_POSICION = "";
											}
											llaveItem.VALE = "";
											llaveItem.VALE_NUM = itemsOC[b].NAME1;
											//llaveItem.DE_DOC_MATERIAL = itemsOC[b].BLDAT;
											if (itemsOC[b].BLDAT !== "" && itemsOC[b].BLDAT !== null && itemsOC[b].BLDAT !==
												undefined) {
												llaveItem.DE_DOC_MATERIAL = itemsOC[b].BLDAT;
											} else {
												llaveItem.DE_DOC_MATERIAL = "";
											}
											//llaveItem.DE_POS_DOC_MATERIAL = itemsOC[b].GSBER;
											if (itemsOC[b].GSBER !== "" && itemsOC[b].GSBER !== null && itemsOC[b].GSBER !==
												undefined) {
												llaveItem.DE_POS_DOC_MATERIAL = itemsOC[b].GSBER;
											} else {
												llaveItem.DE_POS_DOC_MATERIAL = "";
											}
											//llaveItem.OC_NUMERO_ORDEN = itemsOC[b].BELNR;
											if (itemsOC[b].BELNR !== "" && itemsOC[b].BELNR !== null && itemsOC[b].BELNR !==
												undefined) {
												llaveItem.OC_NUMERO_ORDEN = itemsOC[b].BELNR;
											} else {
											...
											llaveItem.FEC_VALE = itemsOC[b].CPUDT;
											llaveItem.PRECIO_ING = parseFloat(itemsOC[b].RMWWR.toString().replace(',', ''), 10).toString();
											llaveItem.UND_MED = itemsOC[b].WAERS;
											llaveItem.CODIGO = "";
											llaveItem.DESCRIPCION = "";
											llaveItem.CANTIDAD = "";
											//llaveItem.GUIA_REMISION = itemsOC[b].CPUTM;
											if (itemsOC[b].CPUTM !== "" && itemsOC[b].CPUTM !== null && itemsOC[b].CPUTM !==
												undefined) {
												llaveItem.GUIA_REMISION = itemsOC[b].CPUTM;
											} else {
											...
											oModel.create("/" + this.varTableT_FAC_POS + "", llaveItem, {
												...
												success: function (data) {
													...
													//var texto = "/T_CTR_DETs(EM_RUC='" + data.EM_RUC + "',US_RUC='" + data.US_RUC +
													var texto = "/" + this.varTableT_CTR_DET + "(EM_RUC='" + this.varRucDeLaEmpresa + "',US_RUC='" + data.US_RUC +
														"',BELNR='" + data.OC_NUMERO_ORDEN + "',GJAHR='" + data.DE_POSICION + "')";
		...
		btnAnadirDocSus: function () {
			...
			if (varSelecTipoCarga === "C") {
				varDocumentosObli = "( 01 ó 08 - xml ), ST - pdf";
					...
					press: function () {
						...
						if (varContErrorBinario === 0) {
							if (varContNoEstado === 0) {
								if (this.getView().byId("idComboTipo").getSelectedItem().getKey() === "C") {
									...
									for (var k = 0; k < vartamtblDocOK; k++) {
										...
										if ((opcFiltro === "01" || opcFiltro === "08") && opcFiltroExtension === "text/xml") {
											contTipo = contTipo + 1;
										} else if (opcFiltro === "ST" && opcFiltroExtension === "application/pdf") {
											contTipo = contTipo + 1;
										}
									}
				...
				if (vector.length !== 0) {
					for (var i = 0; i < vector.length; i++) {
						if (vector[i].clistTabDocuTipo === varTipo &&
							...
							vector[i].clistTabDocuNumero === varNumComprobante &&
